...
const produceChartDataFromIssues = (data: IssueData[], since: Date, until: Date): [{[name: string]: number[]}, string[], string[], number] => {
    ...
    const labels: string[] = setLabels(getDateMinusDays(since, -earliestDay), until);
    return [issuesByUserAtDate, userNames, labels, earliestDay];
};
export const produceBarChartDataFromIssues = (data: IssueData[], since: Date, until: Date, users: User[]) => {
...
export const produceCumulativeChartDataFromIssues = (data: IssueData[], since: Date, until: Date, users: User[]) => {
