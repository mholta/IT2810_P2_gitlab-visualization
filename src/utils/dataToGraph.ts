...
const produceChartDataFromCommits = (data: CommitData[], since: Date, until: Date): [{[name: string]: number[]}, string[], string[], number] => {
...
export const produceCumulativeChartDataFromCommits = (data: CommitData[], since: Date, until: Date) => {
    ...
    let chart: ChartData = {
      ...
      userNames.forEach((userName) => {
          commitsByUserAtDate[userName] = commitsByUserAtDate[userName].map((sum => (noCommits: number) => sum += noCommits)(0)) // from https://stackoverflow.com/a/55261098/15324998
        chart = {
          ...chart,
          datasets: [
            ...chart.datasets,
...
export const produceBarChartDataFromCommits = (data: CommitData[], since: Date, until: Date) => {
    const [commitsByUserAtDate, userNames, labels, earliestDay] = produceChartDataFromCommits(data, since, until);
    let chart: ChartData = {
        labels: labels,
        datasets: []
...
const produceChartDataFromIssues = (data: IssueData[], since: Date, until: Date): [{[name: string]: number[]}, string[], string[], number] => {
    const labels: string[] = setLabels(since, until);
              ...
              if (day < earliestDay) {
                ...
                if (issue.assignee?.name) {
                    ...
                    issuesByUserAtDate[issue.assignee.name][day] += 1;
                        ...
                        issuesByUserAtDate[issue.assignee.name] = Array(numberOfDays).fill(0);
                        issuesByUserAtDate[issue.assignee.name][day] = 1;
                    }
                }
...
export const produceBarChartDataFromIssues = (data: IssueData[], since: Date, until: Date) => {
    const [issuesByUserAtDate, userNames, labels, earliestDay] = produceChartDataFromIssues(data, since, until);
    let chart: ChartData = {
        labels: labels,
        datasets: []
      };
...
export const produceCumulativeChartDataFromIssues = (data: IssueData[], since: Date, until: Date) => {
    const [commitsByUserAtDate, userNames, labels, earliestDay] = produceChartDataFromIssues(data, since, until);
    let chart: ChartData = {
        labels: labels,
        datasets: []
      };
